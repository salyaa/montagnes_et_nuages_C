            ...
            for( size_t k(0); k<Nz; ++k){
                ...
                if(a!=i or b!=j or c!=k){//différentes variations du taux d'humidité selon les conditions imposées
                    if (cube[i][j][k].etat_nuage() and not (cube[a][b][c].etat_nuage())){
                     ...
                     if (cube[i][j][k].pluie(Physique::pression_equilibre)){
...
bool Ciel::nuage(unsigned int i, unsigned int j, unsigned int k) const
{
    if (cube[i][j][k].etat_nuage()){
        return true;
    } else {
        return false;}
}
...
bool Ciel::pluie(unsigned int i, unsigned int j, unsigned int k) const
{
    if(cube[i][j][k].pluie(Physique::pression_equilibre)){
        return true;
    } else {
