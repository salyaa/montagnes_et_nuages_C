...
ChampPotentiel::ChampPotentiel(unsigned int Nx, unsigned int Ny, unsigned int Nz,  double echant,
                               double x_, double y_, double h_, double sigma1, double sigma2)
...
void ChampPotentiel::affichePot() const
{
    for(size_t i(0); i < Nx; ++i) {
        for(size_t j(0); j < Ny; ++j) {
            for (size_t k(0); k < Nz; ++k) {
                cout << i << " " << j << " " << k << " " <<setprecision(14)
                     << collection3D[i][j][k].getPotentiel() << endl;
            }
        }
    }
}
...
void ChampPotentiel::resolution(double const& seuil, unsigned int const& nb_ite, bool verbeuse)
{
    ...
    while((erreur()>=seuil) and (r<=nb_ite)){
        ...
        r+=1;
        //DÉCOMMENTER ICI POUR LE TEST ERREUR
        //cout << r << " " << erreur() << endl;
    }
}
...
double ChampPotentiel::norme3D_2(vector<double> test) const { //calcul de laa norme au carré
    double t;
